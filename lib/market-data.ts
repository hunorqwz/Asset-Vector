...
export async function fetchMarketData(ticker: string, len: number = 2500): Promise<MarketSignal> {
  ...
  const [news, fundamentals, tnxQuote, history1h, history15m] = await Promise.all([
    ...
    !isCrypto ? yahooFinance.quoteSummary(ticker, { modules: ['summaryDetail', 'financialData', 'defaultKeyStatistics', 'assetProfile', 'price'] }, { validateResult: false }).catch(() => null) : Promise.resolve(null),
