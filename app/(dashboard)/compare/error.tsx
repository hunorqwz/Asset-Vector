...
export default function CompareError({
  ...
  useEffect(() => {
    console.error("Compare Matrix Error Caught:", error);
  }, [error]);
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8 bg-transparent">
      <div className="max-w-md w-full border border-white/5 bg-[#0f0f11] p-8 rounded-xl flex flex-col items-center text-center">
        ...
        <h2 className="text-[14px] font-bold text-white uppercase tracking-wider mb-2">COMPARISON PIPELINE EXCEPTION</h2>
        <p className="text-[10px] text-zinc-500 font-mono mb-8 uppercase tracking-widest max-w-[280px]">
          An error occurred while compiling metrics for comparison.
        </p>
        <button
          onClick={reset}
          ...
          aria-label="Re-execute compare matrix calculation"
        >
          RECOMPUTE MATRIX
        </button>
      </div>
