...
describe('Inference Engine - Circuit Breaker (lib/inference)', () => {
  ...
  beforeEach(() => {
    ...
    setInCache("ml_failure_count", 0, 0);
    setInCache("ml_circuit_tripped", false, 0);
  ...
  it('should reset failure count on success', async () => {
    ...
    setInCache("ml_failure_count", 2, 60000);

    await predictNextHorizon(dummySequence, tickerSuccess);
    
    expect(getFromCache("ml_failure_count")).toBe(0);
  });
  ...
  it('should trip the circuit after 3 failures', async () => {
    ...
    // Call 1
    await predictNextHorizon(dummySequence, tickerFail);
    expect(getFromCache("ml_failure_count")).toBe(1);
    expect(getFromCache("ml_circuit_tripped")).toBeFalsy();

    // Call 2
    await predictNextHorizon(dummySequence, tickerFail);
    expect(getFromCache("ml_failure_count")).toBe(2);
    expect(getFromCache("ml_circuit_tripped")).toBeFalsy();

    // Call 3 -> Should trip
    await predictNextHorizon(dummySequence, tickerFail);
    expect(getFromCache("ml_failure_count")).toBe(3);
    expect(getFromCache("ml_circuit_tripped")).toBe(true);
  });

  it('should bypass fetch immediately if circuit is tripped', async () => {
    setInCache("ml_circuit_tripped", true, 60000);
    
    const result = await predictNextHorizon(dummySequence, 'ANY_TICK');
    
    // Fetch should not have been called
    expect(fetch).not.toHaveBeenCalled();
    expect(result.source).toContain('PoT Fallback');
  });
