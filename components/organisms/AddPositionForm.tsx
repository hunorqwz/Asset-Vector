...
export function AddPositionForm({ onSuccess }: AddPositionFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            required
            placeholder="NVDA"
            className="w-full bg-black/40 border border-white/10 px-4 py-3 text-sm text-white font-mono rounded-lg focus:outline-none focus:border-matrix/40 focus:ring-1 focus:ring-matrix/30 transition-all uppercase"
          />
        </div>
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            className="w-full bg-black/40 border border-white/10 px-4 py-3 text-sm text-white font-mono rounded-lg focus:outline-none focus:border-matrix/40 focus:ring-1 focus:ring-matrix/30 transition-all"
          />
        </div>
      </div>
      ...
      {error && (
        <div className="flex items-center gap-3 bg-bear/5 border border-bear/20 px-4 py-3 rounded-lg animate-in fade-in">
      ...
      <button
        type="submit"
        disabled={loading}
        className="relative py-3 bg-matrix text-black font-black uppercase text-[12px] tracking-[0.3em] overflow-hidden group rounded-lg hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50 mt-2"
      >
