    ...
    <div className="bg-white/[0.02] border border-white/5 rounded-xl overflow-hidden group">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12">
        ...
        <div className="xl:col-span-3 border-r border-white/5 p-8 bg-white/[0.01]">
          <h2 className="text-xs font-bold text-matrix uppercase mb-8 flex items-center justify-between">
          ...
          <div className="space-y-10">
            ...
            <div>
              ...
              <span className="text-[10px] font-bold text-matrix uppercase tracking-widest bg-matrix/10 px-2 py-0.5 rounded">
            ...
            {displayRisk.horizonConflicts.length > 0 && (
              <div className="pt-4 border-t border-white/5">
                <p className="text-xs font-bold text-amber-500 uppercase mb-4 flex items-center gap-2">
            ...
            {displayRisk.systemWarnings && displayRisk.systemWarnings.length > 0 && (
              <div className="pt-4 border-t border-white/5">
                <p className="text-xs font-bold text-bear uppercase mb-4 flex items-center gap-2">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
                ...
                <div className="space-y-3">
                  {displayRisk.systemWarnings.map((warning, i) => (
                    <div key={i} className="text-[11px] text-zinc-400 font-medium border-l-2 border-bear/50 pl-3">
                      {warning}
                    </div>
                  ))}
                </div>
              ...
              <div className="pt-4 border-t border-white/5">
                ...
                <div className="space-y-3">
                  {displayRisk.correlationAlerts.slice(0, 2).map((alert, i) => (
                    <div key={i} className="text-[11px] text-zinc-400 font-medium border-l-2 border-amber-500/30 pl-3">
                      {alert}
                    </div>
                  ))}
                </div>
        ...
        <div className="xl:col-span-9 p-8">
          <div className="flex items-end justify-between mb-10">
            <div>
              <h2 className="text-xs font-medium text-zinc-400 mb-2">Scenario Risk Analysis</h2>
              <h3 className="text-2xl font-bold tracking-tightest leading-none">Historical Macro Stress Simulations</h3>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {displayRisk.scenarios.map((s, i) => (
              <div key={i} className={`glass-card p-6 border transition-all duration-300 relative overflow-hidden group/scenario ${isDiff ? 'bg-matrix/5 border-matrix/20' : 'bg-white/[0.01] border-white/5 hover:bg-white/[0.03]'}`}>
                ...
                <h4 className="text-sm font-bold text-white tracking-wide mb-2">{s.name}</h4>
                <p className="text-[11px] text-zinc-400 font-medium mb-6 leading-relaxed">{s.description}</p>
                
                <div className="flex items-baseline justify-between pt-4 border-t border-white/5">
                  <span className="text-xs font-medium text-zinc-400">Projected Impact</span>
          ...
          <div className="mt-10 p-4 border border-white/5 bg-white/[0.02] rounded-lg flex items-center gap-4">
             <div className="text-xs bg-matrix/20 text-matrix px-1.5 py-0.5 rounded font-medium shrink-0">BETA ALIGNMENT</div>
          ...
          <div className="mt-6 border-t border-white/5 pt-8">
            ...
            <form onSubmit={handleSimulate} className="flex gap-4 items-end">
              <div className="flex-1 max-w-[200px]">
                <label className="block text-xs font-medium text-zinc-400 mb-2">Ticker</label>
                <input 
                  type="text" 
                  value={simTicker} 
                  onChange={e => setSimTicker(e.target.value)}
                  placeholder="e.g. TLT" 
                  className="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-2.5 text-sm font-mono text-white focus:outline-none focus:border-matrix/40 focus:ring-1 focus:ring-matrix/30 transition-all"
                />
              </div>
              <div className="flex-1 max-w-[200px]">
                <label className="block text-xs font-medium text-zinc-400 mb-2">Simulated Capital ($)</label>
                <input 
                  type="number" 
                  value={simAmount} 
              ...
              <button 
                ...
                className="bg-white text-black font-bold tracking-wide text-sm px-8 py-3 rounded-md hover:bg-zinc-200 transition-colors disabled:opacity-50"
              >
              ...
              {isDiff && (
                <button 
                  ...
                  className="bg-transparent border border-white/20 text-zinc-400 font-bold tracking-wide text-sm px-4 py-3 rounded-md hover:text-white transition-colors"
                >
                  Reset
                </button>
              )}
